...
pub async fn search_accounts(
    ...
    let min_len_ok =
        viewer_id.is_some() || query.chars().count() >= MIN_ACCOUNT_QUERY_LENGTH;
    ...
    if min_len_ok && remaining > 0 {
        ...
        let terms = if domain_is_local { &username_part } else { &query };
        let tsquery = account_tsquery(terms);
        let ranked = match viewer_id {
            Some(vid) => {
                advanced_account_search(&state, &tsquery, vid, following, remaining, offset)
                    .await?
            }
