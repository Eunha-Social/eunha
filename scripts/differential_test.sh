...
TOKEN=$(docker compose -f "$COMPOSE" exec -T web bin/rails runner '
  account = Account.find_or_create_by!(username: "differ") { |a| a.domain = nil }
  user = User.find_by(email: "diff@localhost") || User.create!(
    email: "diff@localhost", password: SecureRandom.hex(16), account: account,
    agreement: true, approved: true, confirmed_at: Time.now.utc)
  user.update!(approved: true, confirmed_at: Time.now.utc)
  app = Doorkeeper::Application.find_or_create_by!(name: "differential") do |a|
